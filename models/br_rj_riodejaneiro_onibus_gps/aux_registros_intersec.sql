...
times AS ( 
	...
	JOIN
		UNNEST (GENERATE_TIMESTAMP_ARRAY(r.min_date,
			r.max_date,
		INTERVAL {{ var('faixa_horaria_minutos') }} minute) ) faixa_horaria ),
faixas AS ( 
	...
	ON
		...
		AND datetime(TIMESTAMP_ADD(faixa_horaria, INTERVAL {{ var('faixa_horaria_minutos') }} minute))) ),
intersects AS ( 
	SELECT
		id_veiculo,
		...
		COUNT(CASE
			WHEN st_dwithin(posicao_veiculo_geo, shape, {{ var('tamanho_buffer_metros')}}) THEN 1
		END
		) n_intersec,
		...
		WHEN COUNT(CASE
			...
			posicao_veiculo_geo,
			{{ var('buffer_inicio_fim_metros') }}) IS TRUE THEN 1
		END
		)>=1 THEN 'start'
		WHEN COUNT(CASE
	...
	JOIN
		{{ source('br_rj_riodejaneiro_sigmob', 'shapes_geom') }} s
	ON
...
SELECT
  *,
  STRUCT("lalala" AS versao_maestro,
    "lelele" AS versao_maestro_bq) versao
FROM
  intersects
WHERE
  n_intersec > 0